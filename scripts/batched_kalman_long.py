...
#        accum_batch = 30
    ...
    elif dataset == 'ownTemporal':
        valdir =  '/data/hinthorn/workspace_hinthorn/exp/sequence/freespace_temporal/list_val/'
        traindir =  '/data/hinthorn/workspace_hinthorn/exp/sequence/freespace_temporal/list_train/'
    else:
        raise(RuntimeError("{} Not a legal dataset".format(dataset)))
    ...
    val_loader = torch.utils.data.DataLoader(
        RNNLoader(valdir, valroot, aug_type=2, tosize=dims, lbsize=dims, ignore_label=-1, lb_type='mixed', rand_seq_len =('full', -1), shuffleInput=False, mode='val'), 
...
def train(train_loader, model, criterion, criterion2, optimizer, train_style, epoch):
    ...
    for k in range(batch_size):
        ...
        for i, (imgnames, targnames, scale, hval, wval, reflect) in enumerate(train_loader):
            ...
            batch_len = 1 #8
            if SEQLENGTH % batch_len != 0:
                raise(RuntimeError("Sequence length must be multiple of batch length"))
            batches = SEQLENGTH // batch_len
            ...
            hi, prev, acc = None, None, None #Initialize
            #Train by seqence
            for b in range(batches):
                input_var = torch.cat(inputs[b*batch_len:(b+1)*batch_len],0)
                target_var = torch.cat(targets[b*batch_len:(b+1)*batch_len],0)
                out = None
                if train_style == 'kalman':
                    startTime = time.time()
                    #out, out2, out3 = model(input_var) #input hidden layer  
                    out, prev, hi, acc = model(input_var, prev, hi, acc) #input hidden layer  
                    if prev and acc:
                        prev, hi, acc = prev.detach(), hi.detach(), acc.detach()
                    #Cut off in batches to reduc memory footprint
                ...
                loss1 = loss1 / float(accum_batch//SEQLENGTH)
...
def validate(val_loader, model, criterion, train_style, epoch, num_labels=2):
    ...
    for i, (imgnames, targnames, scale, hval, wval, reflect) in enumerate(val_loader):
            ...
            for b in range(batches):
                ...
                out, prev, hi, acc =  model(input_var, prev, hi, acc)
                if prev and hi and acc:
                    prev, hi, acc = prev.detach(), hi.detach(), acc.detach()
